...
//! topic transitions from "nobody here wants this" to "somebody
//! does" (or back). See ADR-0011.
...
/// node's connections are currently interested in each topic.
///
...
/// own subscriptions. A topic gaining its first interested connection
/// (from *any* client or peer) is what should trigger telling every
/// other active peer link about it; losing its last is what should
/// trigger telling them to stop (see ADR-0011).
#[derive(Debug, Default, Clone)]
pub struct Interest {
    counts: Arc<Mutex<HashMap<Topic, usize>>>,
}
...
impl Interest {
    ...
    /// Records one more interested connection for `topic`. Returns
    /// `true` if this was the topic's first (a 0 -> 1 transition) -
    /// the caller should propagate a `Subscribe` to its other peer
    /// links only in that case.
    pub fn subscribe(&self, topic: Topic) -> bool {
    ...
    /// Records one fewer interested connection for `topic`. Returns
    /// `true` if this was the topic's last (a 1 -> 0 transition) -
    /// the caller should propagate an `Unsubscribe` to its other peer
    /// links only in that case.
    ///
    /// A no-op returning `false` for a topic with no recorded
    ...
    /// A snapshot of every topic with at least one interested
